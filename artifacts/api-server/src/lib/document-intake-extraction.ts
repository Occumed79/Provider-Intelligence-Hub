...
const OPTIIC_URL = "https://api.optiic.dev/process";
...
function firstConfiguredKey(base: string, maxSlots = 5) {
...
async function llamaCloud(buffer: Buffer, mimeType: string, filename: string) {
  const credential = firstConfiguredKey("LLAMA_CLOUD_API_KEY", 5);
  ...
  if (!create.ok) throw new Error(`LlamaCloud slot ${credential.slot} returned ${create.status}: ${createBody.slice(0, 200)}`);
  ...
  while (Date.now() < deadline) {
    ...
    if (!response.ok) throw new Error(`LlamaCloud status returned ${response.status}: ${body.slice(0, 200)}`);
    ...
    if (job.status === "FAILED" || job.status === "CANCELLED") {
      throw new Error(job.error_message || "LlamaCloud job failed");
    }
  }
...
async function optiic(buffer: Buffer, mimeType: string, filename: string) {
  const credential = firstConfiguredKey("OPTIIC_API_KEY", 4);
  if (!credential) return null;

  const form = new FormData();
  form.append("image", new Blob([new Uint8Array(buffer)], { type: mimeType || "application/octet-stream" }), filename);

  const response = await fetch(OPTIIC_URL, {
    method: "POST",
    headers: { Authorization: `Bearer ${credential.key}` },
    body: form,
  ...
  const body = await response.text();
  if (!response.ok) throw new Error(`Optiic slot ${credential.slot} returned ${response.status}: ${body.slice(0, 200)}`);
  ...
  try {
    ...
    const extracted = parsed.text || parsed.result?.text || parsed.data?.text;
    if (extracted?.trim()) return cleanExtractedText(extracted);
    if (parsed.error || parsed.message) throw new Error(parsed.error || parsed.message);
  } catch (error) {
    if (error instanceof SyntaxError && body.trim()) return cleanExtractedText(body);
    throw error;
  }

  throw new Error("Optiic returned no readable text");
}

async function ocrSpace(buffer: Buffer, mimeType: string, filename: string) {
  const credential = firstConfiguredKey("OCRSPACE_API_KEY", 5);
  if (!credential) return null;
  ...
  const body = await response.text();
  if (!response.ok) throw new Error(`OCR.Space slot ${credential.slot} returned ${response.status}: ${body.slice(0, 200)}`);
  ...
  if (!text) throw new Error("OCR.Space returned no readable text");
  return text;
}
...
async function cloudCascade(buffer: Buffer, mimeType: string, filename: string, fileType: string) {
  const attempts = fileType === "pdf"
    ? [
        ...
        ["cloud_optiic", () => optiic(buffer, mimeType, filename)],
  ...
  return errors.length
    ? { text: "", mode: "metadata_only" as const, warning: errors.join(" | ").slice(0, 1000) }
    : null;
}
...
async function parsePdf(file: Express.Multer.File): Promise<ExtractionResult> {
  ...
  try {
    ...
    return {
      text: embedded,
      mode: "local_pdf",
      warning: cloud?.warning || "No readable PDF text was recovered",
    };
  } finally {
...
export async function extractDocumentText(file: Express.Multer.File, fileType: string): Promise<ExtractionResult> {
  try {
    if (fileType === "text") {
      return {
        text: cleanExtractedText(await fs.promises.readFile(file.path, "utf8")),
        mode: "local_text",
      };
    }
    ...
    if (fileType === "spreadsheet") {
      ...
      const chunks = workbook.SheetNames.map(
    ...
    if (fileType === "image") {
      ...
      return {
        ...
        warning: local ? undefined : cloud?.warning || "No readable image text was recovered",
      };
    }
    ...
    return {
      text: "",
      mode: "metadata_only",
      warning: error instanceof Error ? error.message : "Document extraction failed",
    };
  }
}
