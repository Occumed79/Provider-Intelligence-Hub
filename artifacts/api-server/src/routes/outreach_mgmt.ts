...
router.post("/outreach", async (req, res): Promise<void> => {
  ...
  const inserted = await db
    .insert(outreachRecordsTable)
    .values(
      records.map((r) => ({
        providerId: r.providerId as number | undefined,
        providerName: r.providerName as string,
        providerCity: r.providerCity as string | undefined,
        providerState: r.providerState as string | undefined,
        providerEmail: r.providerEmail as string | undefined,
        providerFax: r.providerFax as string | undefined,
        outreachType: (r.outreachType as string) || "email",
        templateName: r.templateName as string | undefined,
        subject: r.subject as string | undefined,
        body: r.body as string | undefined,
        recipientEmail: r.recipientEmail as string | undefined,
        recipientFax: r.recipientFax as string | undefined,
        status: (r.status as string) || "sent",
        followUpDate: r.followUpDate ? new Date(r.followUpDate as string) : undefined,
...
router.patch("/outreach/:id", async (req, res): Promise<void> => {
  ...
  const [updated] = await db
    .update(outreachRecordsTable)
    .set(updates)
    .where(eq(outreachRecordsTable.id, id))
    .returning();
