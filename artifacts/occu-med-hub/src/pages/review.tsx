...
import { CheckCircle, XCircle, AlertTriangle, Clock, ListChecks, Fingerprint, Search } from "lucide-react";
import { format } from "date-fns";
import { motion, AnimatePresence } from "framer-motion";

const REVIEW_STATUSES = [
  { value: "pending", label: "Pending", activeClass: "bg-primary/20 text-white" },
  { value: "in_progress", label: "In Progress", activeClass: "bg-yellow-500/20 text-yellow-100" },
  { value: "approved", label: "Approved", activeClass: "bg-amber-500/20 text-amber-100" },
  { value: "rejected", label: "Rejected", activeClass: "bg-orange-500/20 text-orange-100" },
];
          ...
          <div className="bg-black/60 border border-white/10 rounded-lg h-12 p-1 max-w-2xl flex">
            {REVIEW_STATUSES.map((status) => (
              <button
                ...
                className={`flex-1 rounded-md px-3 py-1 text-sm font-medium transition-all ${activeStatus === status.value ? status.activeClass : "text-muted-foreground hover:text-white hover:bg-white/5"}`}
              >
                {status.label}
              </button>
            ))}
          ...
          {itemsLoading ? (
            ...
            <div className="divide-y divide-white/[0.05]">
              <AnimatePresence mode="popLayout">
                {items.map((item) => (
                  <motion.div key={item.id} layout initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95, transition: { duration: 0.2 } }} className="p-6 hover:bg-white/[0.02] transition-colors group relative overflow-hidden">
                    ...
                    <div className="flex flex-col xl:flex-row gap-6 items-start justify-between">
                      <div className="flex-1 space-y-4 w-full">
                        <div className="flex flex-wrap items-center gap-3">
                          <Badge variant="outline" className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 border shadow-sm ${item.priority === "High" ? "bg-orange-500/20 text-orange-400 border-orange-500/30 shadow-[inset_0_0_10px_rgba(228,114,0,0.1)]" : "bg-white/10 border-white/20 text-muted-foreground"}`}>
                      ...
                      <div className="flex xl:flex-col items-center gap-3 shrink-0 w-full xl:w-auto mt-4 xl:mt-0">
                        {activeStatus === "pending" || activeStatus === "in_progress" ? (
                          <>
                            <Button className="flex-1 xl:w-32 h-12 bg-amber-500/20 hover:bg-amber-500/40 text-amber-400 border border-amber-500/30 shadow-[0_0_15px_rgba(230,180,0,0.1)] transition-all hover:shadow-[0_0_20px_rgba(230,180,0,0.3)] font-bold tracking-wide" onClick={() => handleAction(item.id, "approved")} disabled={updateItem.isPending}>
                              <CheckCircle className="h-4 w-4 mr-2" /> Approve
                            </Button>
                            <Button variant="outline" className="flex-1 xl:w-32 h-12 border-orange-500/30 text-orange-400 bg-orange-500/10 hover:bg-orange-500/20 hover:text-orange-300 font-bold tracking-wide" onClick={() => handleAction(item.id, "rejected")} disabled={updateItem.isPending}>
