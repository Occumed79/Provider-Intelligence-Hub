...
import { Activity, FileText, Building2, Map as MapIcon, AlertTriangle, ArrowUpRight, Upload } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: recentUploads, isLoading: isLoadingUploads } = useGetRecentUploads();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <Geographies geography={geoUrl}>
                  {({ geographies }) =>
                    geographies.map((geo) => {
                      ...
                      // In a real app we'd map name to code or use a lookup, assuming simple map for now
                      const stateData = coverageData?.find(d => d.state === stateCode || d.state === geo.id); // Approximation
                      
                      // Mock visual intensity since we don't have perfect state code mapping in this demo
                      // We'll generate a visual glow based on random for now or just light them up.
                      const isCovered = Math.random() > 0.6; // Mocking coverage visual
        ...
        <div className="flex flex-col gap-4">
          ...
          <StatCard 
            ...
            icon={AlertTriangle} 
            isLoading={isLoadingStats} 
            alert={stats?.providersNeedingReview && stats.providersNeedingReview > 10 ? true : false}
          />
        </div>
      </div>
            ...
            {isLoadingUploads ? (
              ...
              <div className="divide-y divide-white/[0.05]">
                {recentUploads.map(upload => (
                  <div key={upload.id} className="p-4 hover:bg-white/[0.04] transition-colors flex items-center justify-between group">
                    <div className="flex items-start gap-4 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-sm text-white truncate group-hover:text-primary/90 transition-colors">{upload.originalFilename}</p>
                  ...
                  <div key={activity.id} className="relative flex items-start justify-between md:justify-normal md:odd:flex-row-reverse group">
                    ...
                    <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] p-3 rounded-lg bg-white/[0.02] border border-white/[0.05] ml-4 md:ml-0 group-hover:bg-white/[0.04] transition-colors">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-semibold text-xs text-white capitalize tracking-wide">{activity.type}</span>
                        <time className="text-[10px] text-muted-foreground/70">{format(new Date(activity.createdAt), "MMM d, h:mm a")}</time>
                      </div>
                      <div className="text-sm text-muted-foreground leading-relaxed">{activity.description}</div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status.includes("Extracted") || status.includes("Mapped") || status.includes("Approved")) {
    ...
    glowClass = "shadow-[inset_0_0_10px_rgba(230,180,0,0.1),0_0_5px_rgba(230,180,0,0.2)]";
  } else if (status.includes("Failed") || status.includes("Duplicate")) {
