...
function inRange(value: string | null | undefined, range: RangeKey) {
  ...
  return days == null || Date.now() - timestamp <= days * DAY_MS;
}
...
function serviceBreakdown(providers: Provider[]): ServiceCount[] {
  ...
  providers.forEach((provider) => {
    serviceNames(provider.servicesOffered).forEach((service) => {
      const name = service.length > 42 ? `${service.slice(0, 39)}…` : service;
...
function weeklyTrend(providers: Provider[]) {
  ...
  providers.forEach((provider) => {
    ...
    const stamp = new Date(provider.createdAt).getTime();
    if (!Number.isFinite(stamp)) return;
    const weeksAgo = Math.floor((now - stamp) / (7 * DAY_MS));
    if (weeksAgo >= 0 && weeksAgo <= 11) buckets[11 - weeksAgo] += 1;
  });
  return buckets;
}

function statusLabel(value: unknown) {
  return safeText(value, "Needs Review").replaceAll("_", " ");
}
...
export default function NetworkDevelopment() {
  ...
  const rangeProviders = useMemo(() => providers.filter((provider) => inRange(provider.createdAt, range)), [providers, range]);
  const rangeOutreach = useMemo(() => outreach.filter((record) => inRange(record.createdAt || record.sentAt, range)), [outreach, range]);
  ...
  const activeOutreach = outreach.filter((record) => !["signed", "declined"].includes(safeText(record.status).toLowerCase())).length;
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-muted-foreground">Expansion, procurement, and outreach movement — rendered from the live provider data.</p>
          </div>
        </div>
        <div className="inline-flex self-start rounded-xl border border-white/10 bg-black/30 p-1 backdrop-blur-xl">
          {(["week", "month", "quarter", "all"] as RangeKey[]).map((item) => (
            <button
              ...
              className={`rounded-lg px-3 py-2 text-xs font-bold uppercase tracking-wider transition-all ${range === item ? "bg-primary text-primary-foreground shadow-[0_0_22px_rgba(18,173,165,.25)]" : "text-muted-foreground hover:bg-white/5 hover:text-white"}`}
            >
      ...
      <Card className="glass-panel relative overflow-hidden border-primary/20 shadow-[0_20px_80px_rgba(0,0,0,.35)]">
        <CardHeader className="relative z-10 flex flex-row items-center justify-between border-b border-white/[0.05] bg-black/25">
          <div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">Zoom the map, scrub the timeline, or play the selected period as providers activate from Occu-Med HQ.</div>
          </div>
        ...
        <CardContent className="p-0">
          <NetworkVfxMap providers={providers} activeProviderIds={newProviderIds} rangeLabel={rangeLabel(range)} loading={loading} />
        </CardContent>
      </Card>
        ...
        <Card className="glass-panel overflow-hidden border-white/[0.07]">
          ...
          <CardContent className="p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
              ...
              <MiniMetric label="Markets" value={uniqueMarkets} />
              <MiniMetric label="Finalized" value={signedInRange} />
            </div>
          </CardContent>
        </Card>
              ...
              return (
                <div key={`${service.name}-${index}`}>
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.05]">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${Math.max(7, (service.count / max) * 100)}%` }} transition={{ duration: .7, delay: index * .05 }} className="h-full rounded-full bg-gradient-to-r from-primary/30 via-primary/70 to-primary shadow-[0_0_14px_rgba(18,173,165,.35)]" />
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.55fr_.85fr]">
        <Card className="glass-panel overflow-hidden border-white/[0.07]">
          <CardHeader className="border-b border-white/[0.05] bg-black/20">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <CardTitle className="flex items-center gap-2 text-white"><Workflow className="h-5 w-5 text-primary" />Provider Procurement Tracker</CardTitle>
                <div className="mt-1 text-xs text-muted-foreground">Real provider records with verification, pricing signal, services, and activation date.</div>
              </div>
          ...
          <CardContent className="p-0">
            <div className="flex flex-col gap-3 border-b border-white/[0.05] p-4 md:flex-row">
              <div className="relative flex-1">
                ...
                <Input value={providerSearch} onChange={(event) => setProviderSearch(event.target.value)} placeholder="Search provider, market, or service…" className="border-white/10 bg-black/30 pl-9 text-white" />
              </div>
              <div className="flex gap-1 overflow-x-auto rounded-lg border border-white/10 bg-black/30 p-1">
                {([[
                  "all", "All"], ["new", "New"], ["verified", "Verified"], ["needs-review", "Needs Review"]] as Array<[ProcurementFilter, string]>).map(([key, label]) => (
                  <button key={key} type="button" onClick={() => setProcurementFilter(key)} className={`whitespace-nowrap rounded-md px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider transition-colors ${procurementFilter === key ? "bg-white/10 text-white" : "text-muted-foreground hover:text-white"}`}>{label}</button>
                ))}
              </div>
            </div>
              ...
              {filteredProviders.map((provider) => (
                <Link key={provider.id} href={`/providers/${provider.id}`} className="group grid grid-cols-[minmax(0,1.4fr)_minmax(0,.85fr)_auto] gap-3 px-4 py-4 transition-colors hover:bg-white/[0.025] md:grid-cols-[minmax(0,1.3fr)_minmax(0,.75fr)_minmax(0,1.25fr)_auto]">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold text-white transition-colors group-hover:text-primary">{safeText(provider.clinicName, `Provider #${provider.id}`)}</div>
          ...
          <Card className="glass-panel overflow-hidden border-primary/15">
            ...
            <CardContent className="p-5">
              ...
              <div className="mt-5 space-y-2">
                <div className="text-[10px] font-black uppercase tracking-[.2em] text-muted-foreground">Latest movement</div>
                {recentWins.length ? recentWins.map((record) => (
                  <div key={record.id} className="flex items-center gap-3 rounded-lg border border-white/[0.05] bg-black/20 p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-.5 truncate text-[10px] text-muted-foreground">{[safeText(record.providerCity), safeText(record.providerState)].filter(Boolean).join(", ") || safeText(record.outreachType, "Outreach")}</div>
                    </div>
            ...
            <CardContent className="p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">Drop provider documents into the existing intake flow; extracted evidence feeds the same provider records and review queue used by this hub.</p>
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35 }}>
      <div className={`glass-panel relative overflow-hidden rounded-xl border p-4 ${highlight ? "border-primary/25" : "border-white/[0.06]"}`}>
        ...
        <div className="relative flex items-center justify-between gap-3">
          <div><div className="text-[9px] font-black uppercase tracking-[.18em] text-muted-foreground">{label}</div><div className={`mt-1 text-2xl font-black ${highlight ? "text-primary" : "text-white"}`}>{loading ? "—" : value.toLocaleString()}</div></div>
          <div className={`flex h-9 w-9 items-center justify-center rounded-lg border ${highlight ? "border-primary/25 bg-primary/10 text-primary" : "border-white/10 bg-white/[.035] text-white/65"}`}><Icon className="h-4 w-4" /></div>
        </div>
      </div>
    </motion.div>
  );
...
function MiniMetric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-lg border border-white/[0.06] bg-black/20 p-3"><div className="text-xl font-black text-white">{value}</div><div className="mt-.5 text-[9px] font-bold uppercase tracking-wider text-muted-foreground">{label}</div></div>;
}
...
function GrowthSparkline({ values }: { values: number[] }) {
  ...
  const points = values.map((value, index) => ({ x: (index / Math.max(values.length - 1, 1)) * width, y: height - 18 - (value / max) * (height - 38) }));
  const path = points.map((point, index) => `${index === 0 ? "M" : "L"}${point.x.toFixed(1)},${point.y.toFixed(1)}`).join(" ");
  const area = points.length ? `${path} L${width},${height} L0,${height} Z` : "";
  return (
    <div>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-40 w-full overflow-visible" role="img" aria-label="Providers added over the last twelve weeks">
        <defs><linearGradient id="ndhGrowthFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="rgb(18,173,165)" stopOpacity=".3" /><stop offset="100%" stopColor="rgb(18,173,165)" stopOpacity="0" /></linearGradient><filter id="ndhGrowthGlow"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs>
        {[.25, .5, .75].map((ratio) => <line key={ratio} x1="0" x2={width} y1={height * ratio} y2={height * ratio} stroke="rgba(255,255,255,.055)" strokeWidth="1" />)}
        {area && <path d={area} fill="url(#ndhGrowthFill)" />}
        {path && <motion.path d={path} fill="none" stroke="rgb(18,173,165)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" filter="url(#ndhGrowthGlow)" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: .9, ease: "easeOut" }} />}
        {points.map((point, index) => <circle key={index} cx={point.x} cy={point.y} r={2.5} fill="rgb(18,173,165)" stroke="rgba(255,255,255,.8)" strokeWidth=".8" />)}
      </svg>
      ...
      {stages.map((stage, index) => {
        ...
        return (
          <div key={stage.key} className="relative rounded-lg border border-white/[0.06] bg-black/20 p-3 text-center">
            {index < stages.length - 1 && <div className="pointer-events-none absolute left-[68%] top-[25px] z-0 hidden h-px w-[65%] bg-gradient-to-r from-primary/35 to-white/[.04] sm:block" />}
            ...
            <div className="mt-2 h-1 overflow-hidden rounded-full bg-white/[.04]"><motion.div initial={{ width: 0 }} animate={{ width: `${(count / max) * 100}%` }} className="h-full rounded-full bg-primary/70 shadow-[0_0_8px_rgba(18,173,165,.4)]" /></div>
          </div>
        );
      })}
    </div>
