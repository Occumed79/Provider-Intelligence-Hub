...
const CURRENCIES = ["EUR", "GBP", "CAD", "AUD", "JPY", "MXN", "COP", "INR", "PHP", "AED", "SAR", "QAR", "KWD", "BHD", "OMR"];
...
export default function CurrencyFeeSchedule() {
  ...
  const refreshRate = React.useCallback(async () => {
    ...
    try {
      ...
      setStatus(`Live rate loaded: 1 ${currency} = ${nextRate.toFixed(6)} USD`);
    } catch {
      setStatus("Live rate failed. Enter a manual USD rate override below.");
    }
  }, [currency]);
  React.useEffect(() => { refreshRate(); }, [refreshRate]);
  ...
  const exportCsv = () => {
    ...
    const body = rows.map((row) => { const local = Number(row.localFee); const usd = Number.isFinite(local) ? local * activeRate : 0; return [row.category, row.service, currency, row.localFee, activeRate, money(usd), row.notes]; });
    const csv = [header, ...body].map((line) => line.map(csvEscape).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `currency-fee-schedule-${currency}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  };
  return <div className="space-y-6 pb-12"><div><h1 className="text-3xl font-bold tracking-tight text-white">Currency Fee Schedule</h1><p className="text-muted-foreground mt-1">Enter provider fees in local currency, convert them to USD with a free live Frankfurter exchange-rate feed, and export the completed schedule.</p></div>{status && <div className="rounded-lg border border-primary/20 bg-primary/10 p-3 text-sm text-primary">{status}</div>}<Card className="glass-panel border-white/[0.06] bg-black/40"><CardHeader><CardTitle className="text-white">Exchange Rate</CardTitle><CardDescription>Live rates use the free Frankfurter API. Manual override stays available if the rate feed is unavailable.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 md:grid-cols-5 gap-3"><select value={currency} onChange={(e) => setCurrency(e.target.value)} className="bg-black text-white border border-white/10 rounded p-2">{CURRENCIES.map((code) => <option key={code} value={code}>{code}</option>)}</select><Input value={activeRate || ""} readOnly className="bg-black/40 border-white/10 text-white" /><Input value={manualRate} onChange={(e) => setManualRate(e.target.value)} placeholder="Manual USD rate override" className="bg-black/40 border-white/10 text-white" /><Button onClick={refreshRate} variant="outline"><RefreshCw className="h-4 w-4 mr-2" />Refresh Rate</Button><Badge variant="outline" className="justify-center border-white/10 text-muted-foreground">{lastUpdated ? `Updated ${lastUpdated}` : "Not updated"}</Badge></CardContent></Card><Card className="glass-panel border-white/[0.06] bg-black/40"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-white">Fee Schedule Rows</CardTitle><CardDescription>Manual local fees automatically calculate the USD equivalent.</CardDescription></div><div className="flex gap-2"><Button onClick={addRow} variant="outline"><Plus className="h-4 w-4 mr-2" />Add Row</Button><Button onClick={exportCsv} className="bg-primary hover:bg-primary/90"><Download className="h-4 w-4 mr-2" />Export CSV</Button></div></CardHeader><CardContent className="space-y-3"><div className="grid grid-cols-[1.2fr_1.6fr_1fr_1fr_1fr_1.4fr_auto] gap-2 text-[10px] uppercase font-bold tracking-widest text-muted-foreground"><div>Category</div><div>Service / Item</div><div>Currency</div><div>Local Fee</div><div>USD Fee</div><div>Notes</div><div /></div>{rows.map((row) => { const local = Number(row.localFee); const usd = Number.isFinite(local) ? local * activeRate : 0; return <div key={row.id} className="grid grid-cols-[1.2fr_1.6fr_1fr_1fr_1fr_1.4fr_auto] gap-2"><Input value={row.category} onChange={(e) => updateRow(row.id, { category: e.target.value })} className="bg-black/40 border-white/10 text-white" /><Input value={row.service} onChange={(e) => updateRow(row.id, { service: e.target.value })} className="bg-black/40 border-white/10 text-white" /><Input value={currency} readOnly className="bg-black/40 border-white/10 text-muted-foreground" /><Input value={row.localFee} onChange={(e) => updateRow(row.id, { localFee: e.target.value })} className="bg-black/40 border-white/10 text-white" /><Input value={money(usd)} readOnly className="bg-black/40 border-white/10 text-primary font-semibold" /><Input value={row.notes} onChange={(e) => updateRow(row.id, { notes: e.target.value })} className="bg-black/40 border-white/10 text-white" /><Button size="icon" variant="ghost" onClick={() => deleteRow(row.id)}><Trash2 className="h-4 w-4" /></Button></div>; })}</CardContent></Card></div>;
}
