...
export function Layout({ children }: LayoutProps) {
  ...
  const navItems = [
    ...
    { label: "Expansion Brief", href: "/expansion-brief" },
    ...
    <div className="flex h-screen w-full overflow-hidden text-foreground">
      ...
      <aside className={cn(
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)} className={cn(
                ...
                {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary shadow-[0_0_10px_rgba(230,180,0,0.8)]" />}
                {isActive && <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-primary/20 to-transparent pointer-events-none" />}
                {Icon && <Icon className={cn("h-4 w-4 transition-colors shrink-0", isActive ? "text-primary drop-shadow-[0_0_8px_rgba(230,180,0,0.7)]" : "group-hover:text-white")} />}
                <span className="flex-1">{item.label}</span>
                <AnimatePresence>
                  {badge > 0 && (
                    <motion.span key="badge" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} transition={{ type: "spring", stiffness: 500, damping: 25 }} className="relative shrink-0 min-w-[18px] h-[18px] px-1 rounded-full bg-primary text-black text-[9px] font-black flex items-center justify-center shadow-[0_0_8px_rgba(230,180,0,0.7)]">
        ...
        <div className="p-4 border-t border-white/[0.05] relative z-10">
          <Link href="/settings" onClick={() => setSidebarOpen(false)} className={cn(
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative z-10">
        <div className="h-16 flex items-center px-4 md:hidden border-b border-white/[0.05] bg-black/40 backdrop-blur-xl z-30">
          <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(true)} className="text-white hover:bg-white/10"><Menu className="h-5 w-5" /></Button>
          <span className="ml-4 font-semibold text-white tracking-wide glow-text">Occu-Med</span>
          {unreadCount > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="ml-2 h-5 w-5 rounded-full bg-primary text-black text-[9px] font-black flex items-center justify-center shadow-[0_0_8px_rgba(230,180,0,0.6)]">{unreadCount}</motion.span>}
        </div>
